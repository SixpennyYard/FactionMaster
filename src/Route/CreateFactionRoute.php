				...
				if (!$factionRequested instanceof FactionEntity) {
					if (strlen($factionName) >= ConfigManager::getConfig()->get("min-faction-name-length")
						...
						if (!in_array($factionName, (array) Utils::getConfig("banned-faction-name"), true)) {
                            ...
                            $var = Utils::getCountryConfig("countries")->getAll("countries");
                            unset($var[$factionName]);
                            Utils::getCountryConfig("countries")->set("countries", $var);
                            Utils::getCountryConfig("countries")->set("created_countries", $factionName);
                            Utils::getCountryConfig("countries")->save();

                            $player->getInventory()->addItem(CustomiesItemFactory::getInstance()->get("customies:bank")->setCount(2));
                            $player->getInventory()->addItem(CustomiesItemFactory::getInstance()->get("customies:government")->setCount(1));
