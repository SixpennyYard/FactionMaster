...
class MainAPI {
     ...
     * @var int
     */
    public static int $surclaim = 0;
    /**
     * @var array
     */
    protected static array $assault = [];
	/** @var FactionEntity[] */
	...
	public static function submitDatabaseTask(DatabaseTask $task): void {
		self::$main->getServer()->getAsyncPool()->submitTask($task);
	}

    public static function isInAssault(string $player): bool
    {
        if (isset(self::$assault[strtolower(self::getFactionOfPlayer($player)->getName())])) return true;
        return false;
    }
    public static function setInAssault(Player $player, string $AttackOrDefence = "attack"): void
    {
        self::$assault[strtolower(self::getFactionOfPlayer($player->getName())->getName())] = [strtolower($player->getName()), $AttackOrDefence];
    }
    public static function unsetInAssault(string $faction): void
    {
        unset(self::$assault[strtolower($faction)]);
    }
}